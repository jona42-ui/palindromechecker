...
function palindromeCheck1(str) {                  // L1 - create function where we can input a 'string', and then perform following to said string:
  return (                                        // L2 - return () -> will output a boolean value based on the === operator, true or false. 
    str.replace(/[\W_]/g, "").toLowerCase() ===   // L3 - "first version" deletes all non-alpha-numeric elements, and lowercases remaining characters -> "jonathan"
    ...
    str                                           // L4 - "second version" aka the reversed version 
      ...
      .reverse()                                  // L8 - AND then we can reverse the elements within the array -> ["n", "a", "h", "t", "a", "n", "o", "j"]
      .join("")                                   // L9 - and then convert it back into a string -> "nahtanoj"
  );                                              // THEREFORE, "jonathan" === "nahtanoj" -> FALSE 
}

// TEST EXAMPLE
console.log(palindromeCheck2("Race Car_")); // True
console.log(palindromeCheck2("jona_Than")); // False 
...
function palindromeCheck2(str) {                                                   // L1
  ...
  */ 
  ...
  str.toLowerCase() -> "jona_Than"
  .replace(nonAlphaNum, ''); -> "jonthan"
  ...
  .split('') -> creates new array with all characters in the string separated into single characters -> ["n", "a", "h", "t", "a", "n", "o", "j"]
  .reverse() -> reverses the order of the elements in an array -> ["n", "a", "h", "t", "a", "n", "o", "j"]
  .join('') -> convert an array of elements into one string -> "nahtanoj"
  
  
  L5 - reversedVersion === firstVersion   ->    "jonathan" === "nahtanoj"  -> FALSE 
  
  */ 
  
  //<!--------------------------------------START WEB API ----------------------------------------->        
  // WEB API - JAVASCRIPT TO HTML
 window.onload=function() {let submitButtonEvent=document.getElementById("submitButton");          // L1 - create var referencing our HTML element with the id "submitButton"
